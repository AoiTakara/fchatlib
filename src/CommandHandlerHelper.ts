...
export class CommandHandlerHelper {
  ...
  internalLoadPlugin(pluginName: string, commandHandler: CommandHandler) {
    ...
    try {
      ...
      if (strAddedCommands == '') {
        commandHandler.fChatLibInstance.sendMessage(
      ...
      if (ex && (ex as any).code == 'MODULE_NOT_FOUND') {
        ...
        this.commandHandler.fChatLibInstance.sendMessage(
        ...
        this.commandHandler.fChatLibInstance.throwError('!loadplugin', (ex as any).toString(), commandHandler.channelName);
      }
    }
  }
      ...
      try {
        ...
        if (ex && (ex as any).code == 'MODULE_NOT_FOUND') {
          ...
          console.log(
            'The ' +
              plugin.name +
              " plugin doesn't contain a class named " +
              plugin.name +
              ', Channel:' +
              this.commandHandler.channelName
          );
