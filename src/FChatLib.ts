...
export default class FChatLib {
  ...
  private readonly commandListeners = Object.fromEntries(
    Object.values(fchatServerCommandTypes).map((command) => [command, []])
  ) as Record<
    FChatServerCommandType,
  ...
  addCommandListener(
    command: FChatServerCommandType,
    fn: FChatListener<SchemaForCommand<FChatServerCommandType>>
  ): void {
  ...
  private users: Record<
    string,
    SchemaForCommand<
      typeof fchatServerCommandTypes.CHARACTER_ONLINE_LIST
    >["characters"][number]
  > = {};
  ...
  async sendData<T extends FChatClientCommandType>(
    command: T,
    content?: ClientCommandSchema<T>
  ): Promise<void> {
    ...
    if (currentTime - this.lastTimeCommandReceived < this.floodLimit) {
      let timeElapsedSinceLastCommand =
        currentTime - this.lastTimeCommandReceived;
      let timeToWait =
        this.commandsInQueue * this.floodLimit - timeElapsedSinceLastCommand;
  ...
  constructor(configuration: IConfig) {
    ...
    try {
      if (statSync(this.saveFolder + this.saveFileName)) {
        this.channels = new Map(
          JSON.parse(readFileSync(this.saveFolder + this.saveFileName, "utf8"))
        );
      }
    } catch (err) {
      //Swallow the error
    ...
    if (
      ...
      this.channels.get(this.config.room.toLowerCase()) == null
    ) {
  ...
  async connect(): Promise<void> {
    ...
    this.addCommandListener(
      fchatServerCommandTypes.MESSAGE_RECEIVED,
      this.commandListener
    ); // basic commands + plugins loader, one instance for one bot
    this.addCommandListener(
      fchatServerCommandTypes.CONNECTED,
      this.joinChannelOnConnect
    );

    if (this.config.autoJoinOnInvite) {
      this.addCommandListener(
        fchatServerCommandTypes.CHANNEL_INVITE_RECEIVED,
        this.joinChannelsWhereInvited
      );
    }

    this.addCommandListener(
      fchatServerCommandTypes.SERVER_VARIABLES,
      this.variableChangeHandler
    );

    //user handling
    this.addCommandListener(
      fchatServerCommandTypes.INITIAL_CHANNEL_DATA,
      this.addUsersToList
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHARACTER_WENT_OFFLINE,
      this.removeUserFromChannels
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHARACTER_LEFT_CHANNEL,
      this.removeUserFromList
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHARACTER_JOINED_CHANNEL,
      this.addSingleUserToList
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHARACTER_JOINED_CHANNEL,
    ...
    this.addCommandListener(
      fchatServerCommandTypes.CHARACTER_ONLINE_LIST,
      this.addUserListToGlobalState
    );
    this.addCommandListener(
      fchatServerCommandTypes.STATUS_CHANGED_FOR_CHARACTER,
      this.onChangeUpdateUserState
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHARACTER_WENT_OFFLINE,
      this.onChangeUpdateUserState
    ...
    this.addCommandListener(
      fchatServerCommandTypes.CHANNEL_OPERATOR_LIST,
      this.addChatOPsToList
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHANNEL_OPERATOR_ADD,
      this.addChatOPToList
    );
    this.addCommandListener(
      fchatServerCommandTypes.CHANNEL_OPERATOR_REMOVE,
      this.removeChatOPFromList
    );
  ...
  joinChannelsWhereInvited(
    args: SchemaForCommand<
      typeof fchatServerCommandTypes.CHANNEL_INVITE_RECEIVED
    >
  ) {
  ...
  joinNewChannel(channel: string) {
    ...
    this.commandHandlers[channel.toLowerCase()] = new CommandHandler(
      this,
      channel
    );
  ...
  private commandListener(
    args: SchemaForCommand<typeof fchatServerCommandTypes.MESSAGE_RECEIVED>
  ) {
    ...
    if (typeof this.commandHandlers[channel] !== "undefined") {
      try {
        ...
        console.log(ex);
        this.throwError(args, ex.toString(), channel);
      }
    }
  }
  ...
  //user management
  private addUsersToList(
    args: SchemaForCommand<typeof fchatServerCommandTypes.INITIAL_CHANNEL_DATA>
  ) {
    const channel = args.channel.toLowerCase();
    ...
    for (let i in args.users) {
      if (this.usersInChannel[channel].indexOf(args.users[i].identity) == -1) {
        this.usersInChannel[channel].push(args.users[i].identity);
      }
    }
    this.debugLog(
      "Added users to list:",
  ...
  private addSingleUserToList(
    args: SchemaForCommand<
      typeof fchatServerCommandTypes.CHARACTER_JOINED_CHANNEL
    >
  ) {
    const channel = args.channel.toLowerCase();
    if (typeof this.usersInChannel[channel] !== "object") {
  ...
  private removeUserFromList(
    args: SchemaForCommand<
      typeof fchatServerCommandTypes.CHARACTER_LEFT_CHANNEL
    >
  ) {
    const channel = args.channel.toLowerCase();
    if (typeof this.usersInChannel[channel] !== "object") {
    ...
    if (this.usersInChannel[channel].indexOf(args.character) != -1) {
      this.usersInChannel[channel].splice(
        this.usersInChannel[channel].indexOf(args.character),
        1
      );
    }
  ...
  private removeUserFromChannels(
    args: SchemaForCommand<
      typeof fchatServerCommandTypes.CHARACTER_WENT_OFFLINE
    >
  ) {
    //remove if offline
  ...
  private addUserListToGlobalState(
    args: SchemaForCommand<typeof fchatServerCommandTypes.CHARACTER_ONLINE_LIST>
  ) {
  ...
  //permissions
  private addChatOPsToList(
    args: SchemaForCommand<typeof fchatServerCommandTypes.CHANNEL_OPERATOR_LIST>
  ) {
    const channel = args.channel.toLowerCase();
    ...
    for (let i in args.oplist) {
      if (this.chatOPsInChannel[channel].indexOf(args.oplist[i]) == -1) {
        this.chatOPsInChannel[channel].push(args.oplist[i]);
        this.infoLog("Added chatOP to list:", channel, args.oplist[i]);
      }
    }
  }

  private addChatOPToList(
    args: SchemaForCommand<typeof fchatServerCommandTypes.CHANNEL_OPERATOR_ADD>
  ) {
    const channel = args.channel.toLowerCase();
    if (typeof this.chatOPsInChannel[channel] !== "object") {
      this.chatOPsInChannel[channel] = [];
  ...
  private removeChatOPFromList(
    args: SchemaForCommand<
      typeof fchatServerCommandTypes.CHANNEL_OPERATOR_REMOVE
    >
  ) {
    const channel = args.channel.toLowerCase();
    if (typeof this.chatOPsInChannel[channel] !== "object") {
    ...
    if (this.chatOPsInChannel[channel].indexOf(args.character) != -1) {
      this.chatOPsInChannel[channel].splice(
        this.chatOPsInChannel[channel].indexOf(args.character),
        1
      );
  ...
  private variableChangeHandler(
    args: SchemaForCommand<typeof fchatServerCommandTypes.SERVER_VARIABLES>
  ) {
  ...
  sendCommand(
    command: FChatClientCommandType,
    object?: ClientCommandSchema<FChatClientCommandType>
  ): boolean {
    ...
    if (!parsed.success) {
      ...
      for (const issue of parsed.error.issues) {
        ...
        this.errorLog(
          `Issue: ${issueMessage}, Path: ${path}, Code: ${issue.code}`
        );
      }
      return false;
    }
  ...
  isUserChatOP(username: string, channel: string): boolean {
    const sanitizedChannel = channel.toLowerCase();
    return (
      this.getChatOPList(sanitizedChannel).indexOf(username) != -1 ||
    ...
    this.ws.on("message", async (data: Buffer) => {
      ...
      if (data != null) {
        ...
        try {
          ...
          const commandObject = getCommandObjectForCommand(
            command as FChatServerCommandType
          );
          if (!commandObject) {
            this.infoLog(
              "Unknown command, calling generic event listeners:",
              command
            );
            listenerPromises.push(
              ...this.genericEventListeners.map((listener) =>
                listener.call(this, dataString)
              )
            );
          ...
          if (commandListener) {
            this.debugLog(
              "Calling command listener:",
              command,
              parsed.data,
              commandListener
            );
            listenerPromises.push(
              ...commandListener.map((listener) =>
                listener.call(this, parsed.data)
              )
            );
          }
