...
export const FlistKinkSearchCommand = {
    ...
    schema: z.object({
        kinks: z.array(z.string()).describe("The kink IDs to search for. List available here: https://www.f-list.net/json/chat-search-getfields.json?ids=true."),
...
export const PingCommand = {
    ...
    schema: z.undefined().describe(`Sends a ping response to the server. Timeout detection, and activity to keep the connection alive.
...
export const TimeoutCommand = {
    ...
    schema: z.object({
        character: z.string().describe("The character to timeout."),
        time: z.number().min(1).max(90).describe("An integer value of the timeout duration in minutes, being a minimum of one minute, and a maximum of 90 minutes."),
 ...
 */
export function getClientCommand<T extends FChatClientCommandType>(command: T): typeof commandTypeToCommandObjectMap[T] {
 ...
 */
export type ClientCommandSchema<T extends FChatClientCommandType> = z.infer<typeof commandTypeToCommandObjectMap[T]["schema"]>;
